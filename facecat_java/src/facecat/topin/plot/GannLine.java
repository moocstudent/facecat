 ...
 * 甘氏线
 */
public class GannLine extends FCPlot {
    ...
    public ActionType getAction() {
        ...
        listP.addAll(Arrays.asList(new FCPointF[]{firstP, secondP, thirdP, forthP, fifthP, sixthP, seventhP, eighthP}));
        if ((x2 > x1 && mp.x >= x1 - 2) || (mp.x <= x1 + 2 && x2 < x1)) {
            int listSize = listP.size();
            for (int i = 0; i < listSize; i++) {
                selected = selectLine(mp, startP.x, startP.y, listP.get(i).x, listP.get(i).y);
                if (selected) {
                    action = ActionType.MOVE;
                    return action;
                }
