...
public class FCClientService {
     ...
     *
     */
    private static java.util.HashMap<Integer, FCClientService> m_services = new java.util.HashMap<Integer, FCClientService>();
    ...
    public static void getServices(ArrayList<FCClientService> services) {
        for (FCClientService service : m_services.values()) {
    ...
    public void onCallBack(FCBinary br, int socketID, int localSID, int len) {
        try {
            ...
            if (compressType == COMPRESSTYPE_GZIP) {
                unzipBody = FCStr.unGZip(body);
                bodyLength = unzipBody.length;
                message = new FCMessage(getGroupID(), getServiceID(), functionID, sessionID, requestID, socketID, state, compressType, bodyLength, unzipBody);
                //TODO...
            } else {
                message = new FCMessage(getGroupID(), getServiceID(), functionID, sessionID, requestID, socketID, state, compressType, bodyLength, body);
            }
            onReceive(message);
