#include "..\\..\\stdafx.h"
#include "..\\..\\include\\xml\\FCUIEvent.h"
...
namespace FaceCat{
    ...
    String FCUIEvent::callFunction(Object sender, int eventID, Object pInvoke){
		...
		if(uiEvent){
			...
			if(uiEvent->m_events.find(control) != uiEvent->m_events.end()){
				FCEventInfo *eventInfo = uiEvent->m_events[control];
    ...
    void FCUIEvent::callInvoke(Object sender, Object args, Object pInvoke){
	...
	void FCUIEvent::addEvent(FCView *control, const String& eventName, const String& function){
        ...
        if (eventID != -1){
			...
			if(m_events.find(control) != m_events.end()){
				eventInfo = m_events[control];
			}
			else{
			...
			case FCEventID::ADD:{
                    ...
                    control->addEvent(cEvent, eventID, this);
                    break;
                }
            ...
            case FCEventID::MENUITEMCLICK:{
                    FCMenuItemTouchEvent cEvent = &callMenuItemClick;
                    control->addEvent(cEvent, eventID, this);
                    break;
                }
