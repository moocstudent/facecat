...
namespace FaceCat{
	...
	void MonthDiv::onClick(FCTouchInfo touchInfo){
        ...
        int monthFCButtonmSize = (int)m_monthButtons_am.size();
		...
		if (m_calendar){
			if(m_monthButtons.size() == 0 || m_monthButtons_am.size() == 0){
                ...
                for (int i = 0; i < 12; i++){
                    ...
                    MonthButton *monthFCButtonm = new MonthButton(m_calendar);
                    monthFCButtonm->setMonth(i + 1);
                    monthFCButtonm->setVisible(false);
                    m_monthButtons_am.add(monthFCButtonm);
                }
			}
		}
	}
