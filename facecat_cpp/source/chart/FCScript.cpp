#include "..\\..\\stdafx.h"
#include "..\\..\\include\\chart\\FCScript.h"
#include <functional>

namespace FaceCat{
	static wstring VARIABLE;
	static wstring VARIABLE2;
	static wstring VARIABLE3;
	static wstring FUNCTIONS;
	static wstring FUNCTIONS_FIELD;
	...
	wstring CVar::getText(FCScript *indicator, CVariable *name){
		if (m_type == 1){
	...
	void FCScript::analysisVariables(wstring *sentence, int line, wstring funcName, wstring fieldText, bool isFunction){
		ArrayList<wstring> wordsList;
        int splitWordsSize = 0;
		wstring *splitWords = splitExpression2(*sentence, &splitWordsSize);
        for(int s = 0; s < splitWordsSize; s++){
            wstring wStr = splitWords[s];
			ArrayList<wstring> subWStr = FCStr::split(wStr, VARIABLE2);
            int subWStrSize = (int)subWStr.size();
            for (int u = 0; u < subWStrSize; u++){
				ArrayList<wstring> sunWStr = FCStr::split(subWStr.get(u), L":");
				int sunWStrSize = (int)sunWStr.size();
				for(int w = 0; w < sunWStrSize; w++){
					if (m_functions.containsKey(sunWStr.get(w))){
						wordsList.add(sunWStr.get(w));
					}
				}
				sunWStr.clear();
            }
        ...
        for (int f = 0; f < wordsListSize; f++){
			...
			while (bIndex != -1){
				...
				for(int s = 0; s < stsize; s++){
					wstring ch = sentence->substr(s, 1);
					if (idx >= bIndex){
						if (ch == L"("){
							count++;
						}
				...
				wchar_t szTemp[20] = {0};
				_stprintf_s(szTemp, 19, L"%d", m_variables.size());
				...
				if (funcID == FUNCTIONID_FUNCTION){
                    if (m_tempFunctions.containsKey(fName)){
                        if ((int)m_tempFunctions.get(fName)->m_fieldText.length() > 0){
							ArrayList<wstring> fieldTexts = FCStr::split(m_tempFunctions.get(fName)->m_fieldText, VARIABLE2);
                            ArrayList<wstring> transferParams = FCStr::split(subSentence, VARIABLE2);
                            subSentence = L"";
	...
	void FCScript::analysisScriptLine(wstring line){
		...
		if (FCStr::toUpper(strLine).find(L"FUNCTION ") == 0){
            int cindex = strLine.find(L"(");
			funcName = strLine.substr(9, cindex - 9);
			int rindex = strLine.find(L")");
			if (rindex - cindex > 1){
				fieldText = strLine.substr(cindex + 1, rindex - cindex - 1);
				ArrayList<wstring> pList = FCStr::split(fieldText, VARIABLE2);
				int pListSize = (int)pList.size();
				for (int i = 0; i < pListSize; i++){
					wstring str = pList.get(i);
					if(str.find(L"[REF]") != -1){
						str = str.substr(5);
					}
		...
		if (op == L":="){
			...
			sentence = strLine;
			ArrayList<wstring> strs = FCStr::split(sentence, VARIABLE2);
			if (strs.size() > 1){
				wstring strVar = strs.get(0);
				sentence = strVar;
		...
		if (followParameters.length() > 0){
			...
			if (followParameters.find(L"COLORSTICK") != -1){
				...
				newLine = L"DRAWICON(1" + VARIABLE2 + variable + VARIABLE2 + L"POINTDOT" + VARIABLE2 + L"DRAWTITLE)";
			}
			else{
	...
	Long FCScript::getColor(const wstring& strColor){
		if(strColor == L"COLORRED") return FCColor::argb(255, 0, 0);
		else if(strColor == L"COLORGREEN") return FCColor::argb(0, 255, 0);
		else if(strColor == L"COLORBLUE") return FCColor::argb(0, 0, 255);
		else if(strColor == L"COLORMAGENTA") return FCColor::argb(255, 0, 255);
		else if(strColor == L"COLORYELLOW") return FCColor::argb(255, 255, 0);
		else if(strColor == L"COLORLIGHTGREY") return FCColor::argb(211, 211, 211);
		else if(strColor == L"COLORLIGHTRED") return FCColor::argb(255, 82, 82);
		else if(strColor == L"COLORLIGHTGREEN") return FCColor::argb(144, 238, 144);
		else if(strColor == L"COLORLIGHTBLUE") return FCColor::argb(173, 216, 230);
		else if(strColor == L"COLORBLACK") return FCColor::argb(0, 0, 0);
		else if(strColor == L"COLORWHITE") return FCColor::argb(255, 255, 255);
		else if(strColor == L"COLORCYAN") return FCColor::argb(0, 255, 255);
			...
			wstring htmlColor = strColor.substr(5);
	...
	float FCScript::getLineWidth(const wstring& strLine){
		float lineWidth = 1;
		if (strLine.length() > 9){
	...
	int FCScript::getMiddleScript(const wstring& script, ArrayList<wstring> *lines){
		wstring value = script;
					...
					if (lineLength == 0){
						...
						if (!isComment){
							...
							if (functionBegin && kh == 1){
								...
								int x = line.rfind(L"))"+ VARIABLE2 + L"ELSE");
	...
	int FCScript::getOperator(const wstring& op){
		if(op == L">=")
			return 5;
		else if(op == L"<=")
			return 8;
	...
	bool FCScript::isNumeric(const wstring& str){
	...
	wstring FCScript::replace(const wstring& parameter){
		int splitParametersLength = 0;
		wstring *splitParameters = splitExpression2(parameter, &splitParametersLength);
		for (int p = 0; p < splitParametersLength; p++){
			wstring str = splitParameters[p];
			if (m_defineParams.containsKey(str)){
				wchar_t szTemp[100] = {0};
				_stprintf_s(szTemp, 99, L"%f", m_defineParams.get(str));
		...
		delete[] splitParameters; //MODIFY
		splitParameters = 0;
		return newParameter;
	}

	CMathElement** FCScript::splitExpression(const wstring& expression, int *sLength){
	...
	wstring* FCScript::splitExpression2(const wstring& expression, int *sLength){
		vector<wstring> lstItem;
		int length = (int)expression.length();
		wstring item = L"";
		wstring ch = L"";
		...
		int lstSize = (int)lstItem.size();
		wstring *exprs = new wstring[lstSize + 1];
		*sLength = lstSize + 1;
	...
	FCScript::FCScript(){
		...
		m_tag = 0;
		ArrayList<wstring> sysFunctions = FCStr::split(FUNCTIONS, L",");
		ArrayList<wstring> sysFunctionsField = FCStr::split(FUNCTIONS_FIELD, L",");
	...
	wstring FCScript::getName(){
		return m_name;
	}

	void FCScript::setName(const wstring& name){
		m_name = name;
	}
	...
	void FCScript::setScript(const wstring& script){
		lock();
		m_lines.clear();
        m_defineParams.clear();
        ArrayList<wstring> lines;
		getMiddleScript(script, &lines);
		int linesCount = (int)lines.size();
        for (int i = 0; i < linesCount; i++){
            wstring strLine = lines.get(i);
			if (strLine.find(L"FUNCTION ") == 0){
				wstring funcName = FCStr::toUpper(strLine.substr(9, strLine.find(L"(") - 9));
				...
				ArrayList<wstring> consts = FCStr::split(strLine.substr(6), L":");
	...
	double FCScript::callFunction(wstring funcName){
		double result = 0;
        lock();
        ArrayList<wstring> lines;
        ...
        for (int i = 0; i < linesSize; i++){
            wstring str = lines.get(i);
            int cindex = str.find(L"(");
            wstring upperName = FCStr::toUpper(str.substr(0, cindex));
            if (m_tempFunctions.containsKey(upperName)){
                CVariable *function = m_tempFunctions.get(upperName);
                int rindex = str.rfind(L")");
                CVariable *topVar = new CVariable();
                topVar->m_indicator = this;
                if (rindex - cindex > 1){
                    wstring pStr = str.substr(cindex + 1, rindex - cindex - 1);
                    ArrayList<wstring> pList = FCStr::split(pStr, VARIABLE2);
                    ArrayList<wstring> fieldTexts = FCStr::split(function->m_fieldText, VARIABLE2);
                    int pListLen = (int)pList.size();
                    topVar->m_parameters = new CVariable*[pListLen * 2];
                    for (int j = 0; j < pListLen; j++){
                        wstring pName = fieldTexts.get(j);
                        wstring pValue = pList.get(j);
                        CVariable *varName = 0;
	...
	wstring FCScript::getText(CVariable *var){
    ...
    CVariable* FCScript::getVariable(const wstring& name){
	...
	void FCScript::removeFunction(CFunction *function){
		...
		void FCScript::setSourceField(const wstring& key, int value){
		CVariable *pfunc = new CVariable();
		pfunc->m_indicator = this;
		pfunc->m_type = 2;
		wchar_t szTemp[20] = {0};
	...
	void FCScript::setSourceValue(int index, const wstring& key, double value){
		CVariable *pfunc = 0;
        for(int v = 0; v < m_variables.size(); v++){
			CVariable *var = m_variables.get(v);
	...
	double FCScript::AVEDEV(CVariable *var){
		...
		double value = avedev(close, list, listLength, avg);
		if(list){
			delete[] list;
			list = 0;
		}
					...
					for (int i = 2; i < var->m_parametersLength; i++){
						...
						if (strParam == L"DRAWTITLE"){
							if (polylineShape->getFieldText().length() != 0){
								m_div->getTitleBar()->Titles.add(new ChartTitle(polylineShape->getFieldName(), polylineShape->getFieldText(), polylineShape->getColor(), 2, true));
							}
						}
					}
				}
	...
	double FCScript::STICKLINE(CVariable *var){
		if (m_div){
			...
			if (!var->m_barShape){
				...
				barShape->setLineWidth(FCStr::convertStrToFloat(width->m_expression.c_str()));
    ...
    int FCScript::STR_FIND(CVariable *var){
        return getText(var->m_parameters[0]).find(getText(var->m_parameters[1]));
    }
    ...
    int FCScript::STR_FINDLAST(CVariable *var){
        return getText(var->m_parameters[0]).rfind(getText(var->m_parameters[1]));
    }
    ...
    int FCScript::STR_SUBSTR(CVariable *var){
        ...
        if (pLen == 2){
            ...
            m_resultVar.m_type = 1;
            m_resultVar.m_str =  L"'" + getText(var->m_parameters[0]).substr((int)getValue(var->m_parameters[1]), (int)getValue(var->m_parameters[2])) + L"'";
        }
        return 0;
    }
    ...
    int FCScript::STR_SPLIT(CVariable *var){
        ...
        if (m_tempVars.containsKey(id)){
            ...
            list->clear();
            ArrayList<wstring> strs = FCStr::split(getText(var->m_parameters[1]), getText(var->m_parameters[2]));
